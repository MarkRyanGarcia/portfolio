            ...
            <div key={idx} className="w-full bg-base-2 border border-secondary rounded-lg p-5">
                <div className="flex flex-col space-y-3">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-1">
                        <div>
                            <h2 className="text-primary font-bold text-xl leading-tight">{item.title}</h2>
                            <h3 className="text-tertiary text-base font-semibold">{item.org}</h3>
                        </div>
                    ...
                    <ul className="flex flex-col gap-1">
                        {item.bullets.map((bullet, bIdx) => (
                            <li key={bIdx} className="text-secondary text-base flex gap-2">
    ...
    return (
        <div className="flex flex-col space-y-5">
            ...
            <div className="flex flex-col w-full space-y-4">
                {isLoading ? <p className="text-secondary text-base">Fetching from resume...</p> : renderItems()}
            </div>
        </div>
    );
}
